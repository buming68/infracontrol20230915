...
/*定义默认IP信息*/
uint8_t local_ip[4]  ={192,100,2,253};						/*定义W5500默认IP地址*/
uint8_t subnet[4]    ={255,255,255,0};						/*定义W5500默认子网掩码*/
uint8_t gateway[4]   ={192,100,2,254};						/*定义W5500默认网关*/
uint8_t dns_server[4]={192,100,0,2};						/*定义W5500默认DNS*/

uint16_t local_port=5000;	                       			/*定义本地端口*/

/*定义远端IP信息*/
uint8_t  remote_ip[4]={192,100,2,11};						/*远端IP地址*/
...
*@return	无
*/
uint8 IINCHIP_READ(uint32_t addrbsb)
{
   uint8 data = 0;
   iinchip_csoff();
   IINCHIP_SpiSendData( (addrbsb & 0x00FF0000)>>16);
...
*@return	len：返回字符串长度
*/
uint16 wiz_write_buf(uint32_t addrbsb,uint8_t * buf,uint16_t len)
{
   uint16 idx = 0;
   if(len == 0) ;
   iinchip_csoff();
...
*@return	len：返回字符串长度
*/
uint16 wiz_read_buf(uint32_t addrbsb, uint8_t * buf, uint16_t len)
{
  uint16 idx = 0;
  if(len == 0)
  {
